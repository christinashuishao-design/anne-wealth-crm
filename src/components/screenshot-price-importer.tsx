...
export function ScreenshotPriceImporter({ products, suppliers }: { products: Option[]; suppliers: Option[] }) {
  const [open,setOpen]=useState(false),[loading,setLoading]=useState(false),[error,setError]=useState(""),[result,setResult]=useState<Result|null>(null);
  async function recognize(form: FormData) {
    ...
    if(!response.ok){setError(json.error||"识别失败");return;} setResult(json.data);
  }
  return <>
    ...
    {open&&<div className="fixed inset-0 z-[70] grid place-items-center bg-black/35 p-4"><div className="max-h-[92vh] w-full max-w-2xl overflow-auto rounded-2xl bg-white">
      ...
      {!result?<form action={recognize} className="space-y-4 p-5"><label className="block text-sm">供应商报价/聊天截图<input className={`${field} file:mr-3`} name="image" type="file" accept="image/*" required/></label>{error&&<p className="rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</p>}<button disabled={loading} className="w-full rounded-xl bg-[#173b34] py-3 text-white disabled:opacity-50">{loading?"正在识别…":"开始识别"}</button></form>:
      <form action={saveRecognizedPrice} onSubmit={()=>setOpen(false)} className="grid gap-4 p-5 sm:grid-cols-2">
        ...
        <button type="button" onClick={()=>setResult(null)} className="rounded-xl border py-3">重新识别</button><button className="rounded-xl bg-[#173b34] py-3 text-white">确认写入价格系统</button>
      </form>}
    </div></div>}
  </>;
}
